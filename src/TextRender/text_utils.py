...
#         print 'line', line_bounds
        ...
        for l in lines:
            ...
            bounds.x = x + bounds.x
            temp = y - bounds.y
            y += bounds.height - bounds.top
            bounds.y = temp
            bbs.append(np.array(bounds))
...
#                     ch_bounds.x = x + ch_bounds.x
#                     ch_bounds.y = y - ch_bounds.y
# #                     print 'actual', (ch_bounds.x, ch_bounds.y)
#                     x += ch_bounds.width + spacing
#                     bbs.append(np.array(ch_bounds))
#                     
#                     temp = ch_bounds.y + ch_bounds.height
#                     if temp > newypos:
#                         newypos = temp
        ...
        if keeponly < 0:
            ...
            rect_union = bbs[keeponly]
        # get the words:
        words = ' '.join(text.split())
        if pad > 0:
            rect_union[:2] -= np.random.randint(pad, size=2)
    ...
    def place_text_at(self, text_arr, mask_shape, bbs, x, y):
        out_arr = np.zeros(mask_shape)
        w,h = text_arr.shape
        loc = np.array([y,x])
        out_arr[loc[0]:loc[0]+w,loc[1]:loc[1]+h] += text_arr
        bbs = move_bb(bbs,loc[::-1])
        return out_arr, loc, bbs, None
...
#             print 'safemask ', type(safemask)
#             print safemask.shape
#             cv2.imshow('safemask', safemask.astype(np.uint8))
#             cv2.waitKey(-1)
